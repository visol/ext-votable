...
';
	 ...
	 */
	public function add($extensionKey, $tableName, $fieldName = 'votes', array $options = []) {
		$didRegister = FALSE;
	...
	protected function addVoteTab($tableName, $fieldName) {
		...
		if (!in_array($tableName, $this->addedVoteTabs)) {
			$fieldList .= '--div--;LLL:EXT:votable/Resources/Private/Language/tx_formule_domain_model_vote.xlf:tabs.votes, ';
	...
	protected function addTcaColumn($tableName, $fieldName, array $options) {
		...
		// @todo https://github.com/lorenzulrich/irreforeignselectordemo

		// Makes sure to add more TCA to an existing structure
		if (isset($GLOBALS['TCA'][$tableName]['columns'])) {
			// Take specific label into account
			$label = 'LLL:EXT:votable/Resources/Private/Language/tx_formule_domain_model_voting.xlf:tx_formule_domain_model_voting';
	 ...
	 * @api
	 */
	static public function getTcaFieldConfiguration($tableName, $fieldName = 'votes', array $fieldConfigurationOverride = []) {
		// Forges a new field, default name is "votes"
		$fieldConfiguration = [
			'type' => 'select',
			'foreign_table' => 'tx_votable_domain_model_voting',
			#'foreign_table_where' => ' AND sys_category.sys_language_uid IN (-1, 0) ORDER BY sys_category.sorting ASC',
			'MM' => 'tx_votable_record_mm',
